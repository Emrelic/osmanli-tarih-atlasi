# KARTVİZİT METİNLERİ — K1 padişahlar, TDV'den yazıldı

**Yazan:** ARAYÜZ 2 · 3 Ağustos 2026 · **Bu bir VERİ teslimidir, koda değil
`data/padisahlar.js`e gider** — o dosya bu oturumun değil (CLAUDE.md §7),
bu yüzden metinler buraya yazıldı; veri oturumu/koordinatör kendi
partisiyle birleştirir.

Sıra koordinatörün istediği gibi **tanınırlığa göre**, ölüm tarihine göre
değil. Her kart TDV'den (`islamansiklopedisi.org.tr`) doğrudan çekilerek
yazıldı, uydurulmadı. `PADISAH-KARTVIZITI.md` şemasına birebir uyuyor:
üç bütçe (künye serbest · magazin ~300 · nasıl-bilirdiniz 900),
`kesinlik:` disiplini, "boş alan yok niçin boş var" kuralı.

---

## 1) II. Mehmed (Fatih) — 872/900

```javascript
{ id:"mehmed2", ad:"II. Mehmed (Fatih)", from:"1451-02", to:"1481-05",
  dogum:"1432-03-30", dogum_yer:"Edirne",
  olum:"1481-05-03", olum_yer:"Hünkârçayırı, Gebze yakını",
  olum_sebep:"sefer yolunda ani ölüm (nikris/gut; zehirlenme iddiası TDV'de doğrulanmadı)",
  baba:"II. Murad", anne:"Hüma Hatun",
  tahta:"1451-02-18", saltanat_yil:30, lakap:["Fâtih"],
  esler:["Sitti Hatun","Gülbahar Hatun"], cocuk:{oglan:3},
...
```
⚠️ **vefat_id çapası:** `olaylar.js`teki mevcut "Fatih'in ölümü — Cem
Sultan olayı" maddesi koordinatörün "ince madde" kararı gereği YENİDEN
YAZILACAK (kendi maddesini alacak) — bu kart o yeni maddeye bağlanmalı,
bugünkü Cem-ağırlıklı maddeye değil.

---

## 2) I. Süleyman (Kanunî) — 795/900

```javascript
{ id:"suleyman1", ad:"I. Süleyman (Kanunî)", from:"1520-09", to:"1566-09",
  dogum:"1494-11-06", dogum_yer:"Trabzon",
  olum:"1566-09-07", olum_yer:"Zigetvar, Macaristan",
  olum_sebep:"kuşatma sırasında otağında, doğal",
  baba:"I. Selim (Yavuz)", anne:"Hafsa Sultan",
  tahta:"1520-09-30", saltanat_yil:46,
  lakap:["Kanunî","Muhteşem (Batı kaynaklarında)"],
  esler:["Mahidevran Hatun","Hürrem Sultan"], cocuk:{oglan:5,kiz:1},
...
```
✅ **vefat_id çapası TEMİZ:** `olaylar.js:76` "Zigetvar — Kanunî'nin
vefatı" — doğrudan bağlanır, yeni madde gerekmiyor.

---

## 3) I. Selim (Yavuz) — 859/900

```javascript
{ id:"selim1", ad:"I. Selim (Yavuz)", from:"1512-04", to:"1520-09",
  dogum:"1470 (dolayı — TDV iki rivayet verir: 1467-68 ya da 1470)",
  dogum_yer:"Amasya",
  olum:"1520-09-21", olum_yer:"Çorlu",
  olum_sebep:"sırtında büyük ur (muhtemelen veba kaynaklı)",
  baba:"II. Bayezid", anne:"Ayşe Hatun (Dulkadıroğlu)",
  tahta:"1512-04-24", saltanat_yil:8, lakap:["Yavuz"],
  esler:["Hafsa Sultan"], cocuk:{oglan:1,kiz:4},
  ...
  kaynak:"TDV: selim-i" }
```
🔴 **vefat_id çapası — GERÇEK EKSİK:** Yavuz'un ölüm/cülûs maddesi
yakınında adı geçen bir madde yok (yalnız "Kanunî tahta çıktı" var, Yavuz
adı geçmeden) — `ARAYUZ-3-SARTNAME.md`nin 8 eksiğinden biri, koordinatörün
yazacağı maddeler arasında.

---

## 4) I. Murad (Hüdavendigâr) — 878/900

```javascript
{ id:"murad1", ad:"I. Murad (Hüdavendigâr)", from:"1362-03", to:"1389-06",
  dogum:"1326", dogum_yer:"Bursa",
  olum:"1389-06-15", olum_yer:"Kosova Ovası", olum_sebep:"suikast",
  baba:"Orhan Bey", anne:"Nilüfer Hatun",
  tahta:"1362-03-01", saltanat_yil:27, lakap:["Hüdavendigâr","Gazi Hünkâr"],
  esler:["Gülçiçek Hatun"], cocuk:{oglan:4},
  ...
  kaynak:"TDV: murad-i" }
```
✅ **vefat_id çapası TEMİZ:** `olaylar.js:26` "I. Kosova Savaşı — I.
Murad'ın şehadeti."

---

## 5) I. Bayezid (Yıldırım) — 898/900 ⚠️ tavan yakın, skandal'a yer yok

```javascript
{ id:"bayezid1", ad:"I. Bayezid (Yıldırım)", from:"1389-06", to:"1402-07",
  dogum:"1354", olum:"1403-03-08", olum_yer:"Akşehir",
  olum_sebep:"esarette, doğal",
  baba:"I. Murad", anne:"Gülçiçek Hatun", lakap:["Yıldırım"],
...
```
✅ **vefat_id çapası TEMİZ:** `olaylar_ek3.js:10` "Yıldırım Bayezid'in
esarette ölümü."

---

## 6) II. Abdülhamid — 860/900

```javascript
{ id:"abdulhamid2", ad:"II. Abdülhamid", from:"1876-08", to:"1909-04",
  dogum:"1842-09-21", dogum_yer:"İstanbul",
  olum:"1918-02-10", olum_yer:"Beylerbeyi Sarayı, İstanbul",
  olum_sebep:"bulunamadı — TDV maddesi sebep belirtmiyor",
  baba:"Sultan Abdülmecid", anne:"Tîrimüjgân Kadınefendi",
  tahta:"1876-08-31", saltanat_yil:33, lakap:["Ulu Hakan (taraftarlarınca)"],
...
{ id:"selim2", ad:"II. Selim", from:"1566-09", to:"1574-12",
  dogum:"1524-05-30", dogum_yer:"Topkapı Sarayı, İstanbul",
  olum:"1574-12-13", olum_yer:"İstanbul",
  olum_sebep:"hamamda düşme sonrası inme, ardından ateş ve mide hastalığı",
  baba:"I. Süleyman (Kanunî)", anne:"Hürrem Sultan",
  tahta:"1566-09-24", saltanat_yil:8,
  ...
  kaynak:"TDV: selim-ii" }
```
✅ **vefat_id çapası TEMİZ:** `olaylar.js` "II. Selim'in vefatı ve III.
Murad'ın cülûsu."
...
{ id:"murad4", ad:"IV. Murad", from:"1623-09", to:"1640-02",
  dogum:"1612-07-27", dogum_yer:"İstanbul",
  olum:"1640-02-08", olum_yer:"İstanbul",
  olum_sebep:"nikris (gut) ve sefer yorgunluğu komplikasyonları, 27 yaşında",
  baba:"I. Ahmed", anne:"Kösem Sultan",
  tahta:"1623-09-10", saltanat_yil:17,
  ...
  kaynak:"TDV: murad-iv" }
```
✅ **vefat_id çapası TEMİZ:** `olaylar.js` "IV. Murad'ın ölümü ve Sultan
İbrahim'in cülûsu."
...
{ id:"selim3", ad:"III. Selim", from:"1789-04", to:"1807-05",
  dogum:"1761-12-24", dogum_yer:"İstanbul",
  olum:"1808-07-28", olum_yer:"Topkapı Sarayı, İstanbul",
  olum_sebep:"suikast (Alemdar Mustafa Paşa'nın yeniden tahta çıkarma "
    + "girişimi sırasında, IV. Mustafa'nın emriyle)",
  baba:"III. Mustafa", anne:"Mihrişah Sultan",
  tahta:"1789-04-07", saltanat_yil:18,
...
```
✅ **vefat_id çapası TEMİZ:** `olaylar_ek7.js` "III. Selim öldürüldü"
(1808-07-28 — 1807 hal' maddesiyle KARIŞTIRILMASIN, ayrı madde).

---

## 10) Osman Gazi (I. Osman) — 834/900

```javascript
{ id:"osman1", ad:"Osman Gazi (I. Osman)", from:"1299-01", to:"1326-04",
  dogum:"1257 (dolayı, kesin değil)",
  olum:"1324-08-01", olum_yer:"Bursa yakını",
  olum_sebep:"hastalık (nikris/gut, Osmanlı rivayetine göre)",
  baba:"Ertuğrul Gazi",
  tahta:"1299 (dolayı)", saltanat_yil:27,
  ...
  kaynak:"TDV: osman-i" }
```
✅ **vefat_id çapası TEMİZ:** `olaylar.js` "Osman Gazi'nin vefatı ve
Orhan Bey'in beyliğe geçişi" (1324-08-01) — ⚠️ dikkat: `to:"1326-04"`
Bursa'nın TAM fethi/Orhan'ın devri tarihidir, Osman'ın ölüm tarihi
DEĞİLDİR; kart `olum:"1324-08-01"`e bağlanmalı, `to`ya değil.

---

## 🔴 Orhan Gazi — TDV'de MÜSTAKİL MADDE BULUNAMADI

Üç slug denendi (`orhan-bey`, `orhan-gazi`, `arama/?q=orhan+gazi`) —
üçü de ya arama sayfası ya da yalnız yapı (câmi/külliye/türbe)
maddelerine düşüyor. TDV'nin Orhan Gazi'nin KENDİSİ için ayrı bir
biyografik maddesi yok görünüyor (Osman Gazi'de `osman-i` vardı, Orhan'da
karşılığı yok). ⚠️ **Bu §4'teki "TDV ölü slug tuzağı" değil** — sayfalar
gerçek arama sonucu döndürüyor, sessizce yanıltmıyor; yalnız KAYNAK YOK.
CLAUDE.md kaynak kuralı gereği ("TDV'nin kapsamadığı ... standart
akademik referans yeterlidir") Orhan Gazi kartı TDV dışı bir kaynakla
yazılmalı — bu ARAYÜZ'ün kararı değil, içerik/veri oturumunun kararı.
Kart bu yüzden SIRADA ATLANDI, listeye not düşüldü.

---

## 11) II. Bayezid (Velî) — 742/900

```javascript
{ id:"bayezid2", ad:"II. Bayezid (Velî)", from:"1481-05", to:"1512-04",
  dogum:"1448", dogum_yer:"Dimetoka",
  olum:"1512-05-26", olum_yer:"Dimetoka yolu",
  olum_sebep:"yolculuk sırasında öldü; zehirlenme ihtimali bazı "
    + "kaynaklarda tartışılır",
  baba:"II. Mehmed (Fatih)", anne:"Gülbahar Hatun",
  tahta:"1481-05-22", saltanat_yil:31, lakap:["Velî"],
...
```
✅ **vefat_id çapası TEMİZ:** `olaylar_ek7.js` "II. Bayezid'in Dimetoka
yolunda ölümü."
