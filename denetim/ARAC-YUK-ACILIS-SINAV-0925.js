// -*- coding: utf-8 -*-
// YUK-BOLME-0925 — ARAC-YUKLEME-0072-SINAV.js KOPYASI. Degisen: URL arguman (argv[3]),
// cikti adi (argv[4]), CDP portu (argv[5]) ve HAZIRLIK OLCUTU: haritaHazir && aktifDonem>=0
// (harita ilk donemi CIZDI — dilimli surumde dilim inmeden cizilmez).
// Kullanim: node denetim/ARAC-YUK-ACILIS-SINAV-0925.js A <url> <etiket> [port+]
...
const URL_YAYIN = process.argv[3] || 'http://localhost:8790/';
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';
const MOD = (process.argv[2] || 'A').toUpperCase();
const CDP = 9350 + (+process.argv[5] || 0);
...
(async () => {
    ...
    for (let i = 0; i < 3000; i++) {           // en cok 300 sn
      ...
      const r = await cagir('Runtime.evaluate', {
        expression: '(function(){try{'
          ...
          + ' hz:(typeof haritaHazir!=="undefined"&&haritaHazir&&typeof aktifDonem!=="undefined"&&aktifDonem>=0),simdi:Math.round(performance.now())});'
      ...
      if (dur.hz) { haritaHazir = dur.simdi; break; }
    }
    await bekle(3000);
  ...
  const dosya = path.join(KOK, 'denetim', 'YUK-BOLME-0925-SINAV-' + (process.argv[4] || MOD) + '.json');
